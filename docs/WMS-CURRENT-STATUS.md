...
- **PRODUCTION LIVE ON VPS (updated 2026-09-07).** WMS is healthy in Docker on the Hostinger VPS (`82.29.175.206`) behind Caddy automatic HTTPS. PR #114 was merged and deployed as `e35c11d3b769e2290c185152cc2479b83f3a98c2`, enhancing the bilingual public landing page's mobile presentation, expanding its workflow story from four to eight controls, and adding production-safe inbound-quality and outbound-goods-issue captures. Authentication remains at `#/login`. Live browser verification passed in English and Arabic with five interactive demo tabs, eight workflow cards, zero horizontal overflow, and zero console errors; public `/healthz` returns `{"status":"ok","service":"wms"}`. The remaining historical Passenger facts below describe the now-defunct shared-hosting deployment; see `docs/HOSTINGER-VPS-MIGRATION-2026-09-06.md` for VPS migration evidence.
...
- Current deployed commit: `e35c11d3b769e2290c185152cc2479b83f3a98c2` (**Verified 2026-09-07** from `/opt/apps/wms`, container health, public assets, and the running mobile landing page).
