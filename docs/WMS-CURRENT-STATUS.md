...
- Latest offsite backup reviewed during PR #53 recovery: GitHub Actions `production-backup.yml` run #15 on 2026-07-31, conclusion **success**. **Reported by the operator, 2026-08-11:** SSH authentication and host-key verification are now fixed (`INC-2026-08-06-01`); a second, independent bug was then found — the workflow's `REMOTE_APP_DIR` pointed at a legacy path with the old, incompatible `better-sqlite3` addon, unrelated to the live app (which was independently confirmed via checksum to be running the correct addon the whole time — production never regressed). That path is fixed in code; the incident stays open until a full green run confirms it.
...
11. **Open, narrowed to one step (`INC-2026-08-06-01`):** the offsite-backup SSH key, host-key, and app-path bugs are all fixed as of 2026-08-11 (operator-executed via Hostinger hPanel, guided remotely — no AI session in this project has ever had production SSH access). Remaining: merge the `REMOTE_APP_DIR` workflow fix and confirm one fully successful (`conclusion: success`) run before closing. Also flagged: the operator's SSH private key was inadvertently pasted into a chat session during troubleshooting and should be rotated as a follow-up hygiene step, independent of the fix above.
