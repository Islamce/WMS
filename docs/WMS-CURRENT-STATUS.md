...
- Latest offsite backup reviewed during PR #53 recovery: GitHub Actions `production-backup.yml` run #15 on 2026-07-31, conclusion **success**. **Verified (repo), 2026-08-06:** every run since (#16–#21, 2026-08-01 through 2026-08-06) has failed — see `INC-2026-08-06-01` in `WMS-INCIDENT-LOG.md`. No offsite backup has succeeded since 2026-07-31.
...
11. **New, open (2026-08-06):** Restore the offsite backup workflow — 6 consecutive daily failures (`INC-2026-08-06-01`), first `/sbin/nologin` (bad remote shell path), then SSH connection timeouts (connectivity/account/firewall). Requires operator action via Hostinger hPanel; no AI agent in this environment has production SSH access to fix it directly. Per `DEC-008`, local retention keeps only 7 backup sets, so this is a growing DR-risk exposure until resolved.
