...
- Latest offsite backup reviewed during PR #53 recovery: GitHub Actions `production-backup.yml` run #15 on 2026-07-31, conclusion **success**. **Verified (repo), 2026-08-09:** every run since (#16–#24, 2026-08-01 through 2026-08-09, 9 consecutive days) has failed — see `INC-2026-08-06-01` in `WMS-INCIDENT-LOG.md` for the corrected per-run breakdown. No offsite backup has succeeded since 2026-07-31.
...
11. **Open, growing (2026-08-01 → 2026-08-09, 9 consecutive daily failures):** Restore the offsite backup workflow (`INC-2026-08-06-01`). The dominant, persistent cause is `/sbin/nologin` on the backup SSH account (bad remote shell path); intermittent SSH connection timeouts also occur but do not persist. Requires operator action via Hostinger hPanel; no AI agent in this environment has production SSH access to fix it directly. Per `DEC-008`, local retention keeps only 7 backup sets — 9 missed cycles already meets or exceeds that window.
