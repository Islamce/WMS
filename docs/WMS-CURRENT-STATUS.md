...
- **CORRECTIVE PROGRAMME DEPLOYED (2026-09-16).** Production moved from
  ...
  `docs/WMS-EXECUTION-HANDOFF.md`) and PR #137. Release run `35143009655`,
  preceded by two `plan_only` runs (`35141145083`, `35142682335`). Migrations
  27 → 31. **Verified (production)** from the run's own before/after comparison:
  `users=11`, `materials=9746`, `batches=3274`, `requests=8`,
  `request_lines=11`, `permissions=42`, `material_location_stock=3273` — all
  **unchanged**; `roles` 11 → 12 and `role_permissions` 105 → 115, exactly the
  declared delta `{"roles":1,"role_permissions":10}` (029 added
  `site_storekeeper` with 12 grants; 028 removed `stock_in`/`stock_out` from the
  `user` role, which held both). `integrity=ok`. `/healthz` now reports
  `release: 0b56bc39…`, asserted by the gate and re-read publicly afterwards.
  DRIFT: the handoff's STOP 1 assumed production at `76a1420` with 026/027
  unapplied; the first plan run showed it at `b46ed7d` with 027 applied — the
  catch-up backlog it described had already been deployed. The read-only STOP 2
  figures, taken from production by the plan run: `materials_with_both=2843`,
  `fully_explained_by_import=2843`, `holds_real_legacy_stock_too=0`,
  `coalesce_would_return_zero=0` — the simple `COALESCE` is safe.
...
- **Account migration complete for all four websites:** R4C is publicly live over valid HTTPS; its public landing update (PR #89, `d7d3e2f`) and authenticated-interface correction (PR #90, `e7f79a6`) are deployed. The latter was verified in a real administrator browser session after rebuilding only `r4c-web-1`; its wider Arabic navigation and unified workspace canvas are live without changing API/database containers or operational data. All 12 R4C migrations remain applied and Postgres/Redis/MinIO are healthy. `kynox.io` and `www.kynox.io` are publicly live over valid HTTPS from the verified static archive, correcting the earlier PHP/MySQL assumption; apex IPv4 and IPv6 both point to the VPS. The separate Hostinger email dispute remains parked with no purchase authorized.
...
13. **RESOLVED (2026-09-04). Was: Open, RECURRED (2026-09-03): shared-hosting process-count pressure.** The `kynox.io` Hostinger shared hosting plan (7 sites) runs Max Processes at 186–200 of a 200 cap, cyclically hitting the ceiling. Investigated 2026-08-31: an SSH session on the host only ever showed 5–6 processes total (one being the WMS Node/Passenger process), and no stuck/parallel deployment was found under `~/domains/r4c.kynox.io` or `~/domains/r4c-api.kynox.io` (checked specifically — both sites' `hbuilds` directories were merely recently touched by normal Passenger release rotation, no active build/git/npm process running, `find -newer` for recent changes came back empty). **Conclusion (2026-08-31): the process count is very likely LVE/CloudLinux-level account-wide resource accounting (PHP-FPM workers, cron, short-lived spawns across all 7 sites) that is not visible from any single interactive SSH session** — further diagnosis requires either Hostinger support's own account-level tooling, or checking each of the other 6 sites' own process/worker configuration individually (not yet done). Suspected (not proven) cause of the three offsite-backup SSH failures on 2026-08-29–31 (`client_loop: send disconnect: Broken pipe`).
    ...
    **Update (2026-09-04, owner asked for the support ticket to be opened directly, hPanel "Agent" identified as live Hostinger Support with backend visibility):** used hPanel's in-panel "Agent" (confirmed via its own reply — "You're already connected to Hostinger Support" — to be the real support channel, not a bot demo; it has backend access to per-site request/error-log metrics not exposed anywhere in the regular hPanel UI). It found no cron jobs anywhere on the account (matches the 09-04 negative-result finding above) and no site whose plain request volume explains the process count, but flagged **`logix.kynox.io` as the best candidate**: 45 requests/24h, 6 server-side 503s, and one request that stayed active for **~942 seconds** — a strong stuck/hung-request signature. Secondary candidate: `gate.kynox.io` (9 requests, 2×503, one 88-second request). `analytics.kynox.io` (130 requests, no errors) and `kynox.io` (most data transferred, no errors) were not implicated. The Agent recommended trying **Restart** on `logix.kynox.io` first (not Delete) — confirmed this would not touch site config/files/database/GitHub link, and since auto-deployment was already off, would not trigger a new deploy.

    **Update (2026-09-04, Restart executed + owner-approved account-wide "Stop running processes" executed — both had only a marginal, non-fixing effect):** Restarted `logix.kynox.io` via hPanel's "Running" status dropdown ("Application restarted." confirmed). Max Processes barely moved: 197 → 193/200 immediately after, still critically high. Reported this to the owner along with a second control found directly on the account's Resources Usage page — **"Stop running processes"** (`hpanel.hostinger.com/websites/kynox.io/order/order-usage`) — whose confirmation dialog reads **"You will temporarily kill all running processes on your account, including active updates, installations, and scripts,"** i.e. account-wide across all 7 sites, not scoped to one site — meaning it could interrupt anything mid-flight on `r4c.kynox.io`/`r4c-api.kynox.io` (under active development by another agent). Given that risk, this was not run without asking; owner explicitly approved ("اوافق واكمل") and it was executed. Result, checked repeatedly over the following ~2 minutes: 193 → 192 → 189/200 — a small, gradual decline, not the sharp drop a genuinely stuck-process fix would produce. **This confirms the prediction already recorded above (2026-09-04 update): killing running/stuck processes is not the fix, because the ceiling is a structural baseline, not a transient stuck-request problem** — with 5 always-on Node.js "Web Apps" sharing one 200-process account cap, the account sits within single digits of the cap essentially by default, so momentarily clearing processes only creates brief headroom that traffic and normal app baseline refill within seconds to minutes. **Practical conclusion:** the two remaining real fixes are unchanged from the 09-04 conclusion above — (a) permanently reduce the number of always-on Node apps sharing the cap, i.e. the owner approving an actual **Delete** of a confirmed-unused site (`logix.kynox.io` is now the best-evidenced candidate, given the ~942s stuck request found by Hostinger's own Agent — though deleting removes it entirely and requires re-import from GitHub plus reconfiguring domain/env vars to undo), or (b) a paid plan upgrade/boost (requires owner's purchase approval, not taken by this session). Auto-deployment for `logix.kynox.io` remains off; the app itself is still running.

    **Update (2026-09-04, owner authorized deleting `logix.kynox.io` and `analytics.kynox.io` — DONE, and this is very likely the actual fix):** owner explicitly instructed deleting both sites outright ("احذف logistics & Analytics"). Both were deleted via hPanel's Websites list → site "⋮" menu → Delete (full-removal confirmation dialog acknowledged and confirmed for each); both showed "Website being deleted" / "was removed" and the account's site count dropped from 7 → 5 (`r4c.kynox.io`, `r4c-api.kynox.io`, `gate.kynox.io`, `wms.kynox.io`, `kynox.io` remain; `r4c`/`r4c-api` untouched throughout, per the standing constraint). Checked Resources Usage ~11 minutes later (past hPanel's own "up to 10 minutes" completion estimate): **Max Processes dropped from the ~189-197/200 baseline to 79/200 (39.5%)** — Memory also dropped from ~570 MB to 238 MB, and PHP Workers from 7 to 4. This is a sharp, large drop — unlike the earlier Restart/"Stop running processes" attempts — and confirms removing two of the five always-on Node apps was the actual structural fix, not merely more headroom that will refill. **Practical consequence: the Max Processes ceiling that has been blocking the offsite backup's SSH connections since 2026-08-29 is very likely resolved.** Next step to fully close this item: confirm the next scheduled `Production Offsite Backup` run (or an on-demand `workflow_dispatch` run) actually succeeds — not yet checked this session. If it does, this item can be marked DONE. Both deleted sites are unrecoverable except by re-importing from GitHub and reconfiguring domain/env vars from scratch, per owner's explicit instruction.

    **RESOLVED (2026-09-04, CONFIRMED by an actual successful backup run).** Triggered an on-demand `workflow_dispatch` run of `Production Offsite Backup` (run `33852928458`, 2026-09-04 08:20 UTC) immediately after the deletions. **Every step succeeded**, including the exact step that had failed instantly on every run since 2026-08-29 (`Snapshot existing manifests`, previously dying in ~14s with `Connection ... closed by remote host` / exit 255 — this time completed normally), through remote backup + restore verification, new-set identification, download, independent runner verification, offsite upload (S3-compatible, HTTPS+SSE), offsite-object verification, and local-retention pruning — full job duration ~1m45s, nothing skipped or soft-failed. **This is the first fully successful, fully verified offsite backup since run #60 on 2026-08-31.** Item 13 is now closed: reducing the account from 7 to 5 sites (deleting `logix.kynox.io` and `analytics.kynox.io`) resolved the Max Processes ceiling that was blocking the backup's SSH connection, and this run proves the fix end-to-end rather than just via the Resources Usage graph. No plan upgrade or paid boost was needed. Going forward, the account still runs 5 always-on Node/PHP sites against the same 200-process cap (now at ~79/200, comfortable headroom) — worth keeping an eye on if `wms`, `r4c`, or `r4c-api` traffic grows materially, but not an active risk today. **Superseded (2026-09-06):** rather than keep relying on that headroom, the operator decided to migrate the whole account off shared hosting onto a dedicated VPS, eliminating the shared process cap entirely. That migration has been executed (refund + VPS purchase + provisioning); see `docs/HOSTINGER-VPS-MIGRATION-2026-09-06.md` for the full record and remaining deployment work. This item's RESOLVED status for the shared-hosting symptom itself stands — it is not reopened — but the shared-hosting platform it describes is being retired.
