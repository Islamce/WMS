...
- **PRODUCTION LIVE ON VPS (updated 2026-09-07).** WMS is healthy in Docker on the Hostinger VPS (`82.29.175.206`) behind Caddy automatic HTTPS. PR #113 was merged and deployed as `bba9823800746d54d03e078a173fd90e019920f9`, adding the bilingual public product landing page at `/` while preserving authentication at `#/login`. Live browser verification passed in English and Arabic with zero console errors; public `/healthz` returns `{"status":"ok","service":"wms"}`. The remaining historical Passenger facts below describe the now-defunct shared-hosting deployment; see `docs/HOSTINGER-VPS-MIGRATION-2026-09-06.md` for VPS migration evidence.
...
- Current deployed commit: `bba9823800746d54d03e078a173fd90e019920f9` (**Verified 2026-09-07** from `/opt/apps/wms` and the running public landing page).
