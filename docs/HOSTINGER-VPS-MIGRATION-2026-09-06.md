...
**Status: IN PROGRESS. VPS purchased, provisioned, and base-configured (updates, firewall, Docker confirmed present, a placeholder Caddy reverse proxy running); both GitHub deploy keys added and verified working. WMS has been cloned and its image built on the VPS but is not running yet because the real JWT secret and the 2026-09-05 SQLite triplet have not been supplied. No application has been deployed and all four sites remain OFFLINE. Hostinger removed the operator's real business email (`islam@kynox.io`) with the cancelled hosting plan; the operator rejected any replacement purchase, and no-charge restoration is under human review as ticket `#20751159` — see §7.**
...
- No application is running on the VPS yet. WMS is cloned at `/opt/apps/wms` and image `wms:905dd74` built successfully, but it remains stopped pending its real JWT secret and the 2026-09-05 `wms.db`/`-wal`/`-shm` files.
...
5. **wms.kynox.io — in progress:** repo cloned to `/opt/apps/wms`; deployment docs read; Compose adapted to bind the app only on `127.0.0.1:3000` and join the external `web` proxy network (commit `905dd74`); Compose validated and the image built successfully on the VPS. The container deliberately remains stopped because the repo correctly rejects a missing/dummy `JWT_SECRET`. To finish, obtain the real JWT secret and the already-downloaded 2026-09-05 `wms.db` + `wms.db-wal` + `wms.db-shm`, restore them into the bind-mounted `data/` directory, start the app, and verify locally before Caddy/DNS changes.
