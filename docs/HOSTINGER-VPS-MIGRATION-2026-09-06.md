...
**Status: IN PROGRESS. VPS purchased, provisioned, and base-configured (updates, firewall, Docker confirmed present); no application has been deployed to it yet; all four sites previously on the shared plan are currently OFFLINE.**
...
3. ~~Set up the VPS base~~ — **done**: firewall (`ufw`, 22/80/443 only), Docker + Docker Compose confirmed present (§4). Not done: unattended-upgrades/patch policy, a non-root deploy user (currently all work is as root via Web Console — acceptable for a single-operator VPS at this stage, revisit if that changes).
4. **r4c-api.kynox.io / r4c.kynox.io**: both repos already ship `docker-compose.yml` / `docker-compose.prod.yml` / `Dockerfile` (per this session's earlier captured notes — not re-verified here) — coordinate with whichever agent/session is actively developing them before redeploying, per item 13's standing caution. Blocked on the operator adding the `vps-deploy-r4c` deploy key (§4) to `Islamce/R4C` on GitHub.
5. **wms.kynox.io**: also ships `docker-compose.yml` + `Dockerfile`, plus `DEPLOY-HOSTINGER.md` and `HOSTINGER-NATIVE-RECOVERY.md` in this repo — read both before redeploying, since they document the prior (Passenger/managed-Node) deployment model that will now change to a VPS/Docker model. Its production database was SQLite on local disk (`data/wms.db`, WAL mode) under the old shared-hosting setup; given §3's resolution (test/demo data, one real account), the plan is simply to restore the already-downloaded `wms.db` as-is on the VPS rather than build a formal migration procedure. Blocked on the operator adding the `vps-deploy-wms` deploy key (§4) to `Islamce/WMS` on GitHub.
...
8. **TLS**: set up a reverse proxy (Nginx or Caddy) with certificates (Let's Encrypt) for all four hostnames once DNS has propagated.
