...
**Status: IN PROGRESS. VPS purchased and base-configured. `wms.kynox.io` is now restored and healthy in Docker on the VPS, including the verified 2026-09-05 SQLite backup; Caddy reaches it through an HTTP-only staging route, but public DNS/TLS cutover is awaiting operator approval, so the public URL remains offline. The other three sites remain undeployed/offline. Hostinger removed the operator's real business email (`islam@kynox.io`) with the cancelled hosting plan; all paid recovery paths were rejected, and the no-charge dispute is recorded in ticket `#20751159` — see §7.**
...
0. **URGENT — recover `islam@kynox.io` (§7).** The operator explicitly refused Hostinger's paid recovery paths. A no-charge entitlement and mailbox-data restoration request is now under human review by Dhita as ticket `#20751159`; do not purchase an email plan or renew Cloud Startup.
...
5. **wms.kynox.io — VPS deployment complete; public cutover pending approval:** located the 2026-09-05 `_data.zip` locally and confirmed it contains the exact `wms.db`/`-wal`/`-shm` triplet; transferred it over a dedicated VPS-only SSH key; matched local/remote SHA-256 (`68fb52080f2adc0e5a377e0bd3d05592b55243a82fd3a2fde38042ce1bc3fd6f`); and validated the restored database (`integrity=ok`, 47 tables). Located the real WMS environment file locally without exposing its 96-character JWT secret. Compose now bind-mounts `./data:/app/data` for auditable restore/backup access (commit `f38be68`), binds the diagnostic port only to VPS localhost, and joins `web`. The container is healthy, reports 20 migrations and 11 users, `/healthz` returns `{"status":"ok","service":"wms"}`, and `/` returns HTTP 200. Caddy has a deliberately HTTP-only `wms.kynox.io` staging route and successfully proxies the same health check. No DNS or TLS change has been made; those require operator approval.
...
**This remains the single most time-sensitive open item in this whole migration** — it affects the operator's real, daily-use email, unlike the four websites whose downtime was pre-accepted. The next proposed compromise is a temporary complimentary one-mailbox entitlement solely to restore/export `islam@kynox.io`, explicitly waiving `no-reply@kynox.io`, while maintaining that no customer-funded purchase is acceptable. If Hostinger agrees, verify both mailbox access and historical data immediately; if it refuses, preserve the written refusal and full ticket record before the 30-day backup deadline.
