...
**Status: IN PROGRESS. VPS purchased and base-configured. `wms.kynox.io` is restored, healthy, publicly live over valid HTTPS, and its offsite-backup workflow is verified against the VPS. `r4c.kynox.io` and `r4c-api.kynox.io` are deployed from merged `main` commit `57f7ac8`, healthy, migrated, seeded, and verified through HTTP-only Caddy staging routes; their DNS/TLS cutover awaits explicit operator approval. `kynox.io` remains offline. The email dispute for `islam@kynox.io` is parked by operator direction with no purchase authorized — see §7.**
...
4. **r4c-api.kynox.io / r4c.kynox.io — DEPLOYED, DNS APPROVAL PENDING:** independent reviewer `sam41234` approved exact RC `705b24f`; PR #88 then merged as `main` commit `57f7ac865fd0ec969f3143cf1783fd21b3b308db`. Cloned that exact merge to `/opt/apps/r4c`, generated independent root-only production secrets with the repository script (a private backup is on the operator's Desktop; no value was printed or committed), and adapted the stack to central Caddy using an untracked VPS Compose override. Postgres 17, Redis 8, and MinIO are healthy; the private `r4c-production` bucket was created; all 12 committed Prisma migrations applied, including `20260903123000_saved_lead_views`; the bootstrap seed created one tenant, one `islam@kynox.io` administrator, four roles, and 95 total permissions. API and web are healthy at localhost ports 4000/3001 and on the external `web` network; HTTP-only Caddy staging routes return API ready (database OK) and the expected web redirect to `/login`. BIM remains disabled. No R4C DNS or public TLS change has occurred; explicit operator approval is the next gate.
