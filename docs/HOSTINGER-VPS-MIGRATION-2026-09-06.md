...
**Status: IN PROGRESS. VPS purchased and base-configured. `wms.kynox.io`, `r4c.kynox.io`, and `r4c-api.kynox.io` are healthy and publicly live over valid HTTPS on `82.29.175.206`; WMS restore/offsite backup and R4C authenticated login/database readiness have been verified. `kynox.io` remains offline and is the final website migration. The email dispute for `islam@kynox.io` is parked by operator direction with no purchase authorized — see §7.**
...
6. **kynox.io — DEPLOYED LOCALLY, DNS APPROVAL PENDING:** the earlier assumption that this required PHP plus the `u716763642_analytics` MySQL database was contradicted by the available backup evidence. The newest usable archive, `public_html-61ecc37.tar.gz` (2026-08-18; SHA-256 `802cd7253651e8beb357eaa7357691d282d8e3b45ec1f7645577022788bc6791`), contains a 31-entry static SPA release (`index.html`, versioned assets, `health.json`, Apache SPA fallback) and no PHP, SQL, `.env`, or database reference. Trusting that concrete artifact, transferred it to `/opt/apps/kynox`, verified the hash again, and deployed it read-only through a healthy `nginx:1.29-alpine` container on localhost port 8080 plus the central `web` network. `/health`, `/`, and a client-route fallback all return HTTP 200; HTTP-only Caddy staging routes for both apex and `www` also pass. The current apex records remain unchanged (`A @` → old `185.97.145.102`; `AAAA @` → old `2a02:4780:27:1522:0:2ab8:f1fa:2`). The new VPS has global IPv6 `2a02:4780:28:7cc3::1`. Explicit approval is required to replace both apex records, after which Caddy can obtain certificates for `kynox.io` and existing CNAME `www.kynox.io`.
