...
## Versioned asset-path correction

The stale-edge diagnosis was confirmed after a successful guarded redeploy: the active Hostinger symlink and candidate files matched the merged hashes, while the public HCDN edge continued returning the older same-path assets even with `Cache-Control: no-cache`, `max-age=0`, and query-string cache busting. The production HTML also retained the older `last-modified` timestamp.

The corrective slice adds a release-scoped `/release-assets/:release/*` static alias backed by the existing `public` directory and changes only the three affected frontend references to `/release-assets/9d79a00/...`. The alias rejects empty, traversal, and nested release paths before delegating to the existing static middleware; it does not alter API routes, authentication, permissions, data paths, migration behavior, or workflow actions. The browser smoke test now verifies the release marker and that the versioned request-detail asset returns HTTP 200 with the queue-continuity marker.

Local checks proved the fresh candidate health endpoint, release marker, versioned asset references, versioned asset response, JavaScript syntax, and `git diff --check`. The broader local `npm test` run remains non-authoritative because this worktree’s local seeded database state is inconsistent with the repository’s deterministic fixtures, producing existing `KeyError`/401 failures in unrelated enterprise suites. CI is the required clean-environment qualification for this correction.
