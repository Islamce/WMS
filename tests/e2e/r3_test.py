#!/usr/bin/env python3
"""Round-3 regressions (requester visibility, live stock, sort/filter,
ABC-XYZ analytics, reallocation, physical inventory, shipping):
 1. ERP / warehouse / picking / GI queues expose requester details.
 2. Material master stock is live (goods receipt updates it immediately).
 3. Materials list supports sort / filters.
 4. Analytics carries ABC / XYZ / FSN / EOQ + the ABC-XYZ matrix.
 5. Reallocation: partial split, reserved guard, permissions, history.
 6. Physical inventory: blind session, freeze, variance approval, posting.
 7. Shipping: eligible -> create -> pack -> load -> dispatch -> deliver + QR label PDF.
Runs in Phase 3 after uat_test (before uat2_test which factory-resets)."""
...
check('picking task list shows requester details',
      row.get('requester_name') and row.get('department') == 'PROD', row)

# drive to GI so the GI queue + shipping can be tested
...
for line in task['lines']:
    call('POST', f"/api/picking/lines/{line['id']}/confirm", picker,
         {'picked_quantity': line['reserved_quantity']})
...
ra_batch = r['batch_id']
c, r = call('POST', '/api/reallocation', requester, {'batch_id': ra_batch, 'quantity': 1, 'to_warehouse': 'WH02'})
check('reallocation blocked without permission (403)', c == 403, (c, r))
c, r = call('POST', '/api/reallocation', supervisor, {'batch_id': ra_batch, 'quantity': 999, 'to_warehouse': 'WH02'})
check('reallocation of more than movable rejected (400)', c == 400 and 'reserved' in r.get('error', '').lower() or c == 400, (c, r))
...
check('partial reallocation splits the batch', c == 201 and r.get('new_batch_id'), (c, r))
new_batch = r.get('new_batch_id')
c, r = call('GET', f'/api/master/batches?search=PO-R3-REALLOC', admin)
src = next((b for b in r['batches'] if b['id'] == ra_batch), {})
dst = next((b for b in r['batches'] if b['id'] == new_batch), {})
...
check('reallocation history recorded', c == 200 and r['total'] >= 1
      and any(mv['to_project'] == 'PRJ-R3' for mv in r['moves']), (c, r.get('total')))

# ===== 6. Physical inventory (annual/periodic) =====
...
check('annual inventory session opened', c == 201 and r.get('lines', 0) >= 1, (c, r))
sid = r.get('id')
...
masked = [l for l in r['lines'] if l['system_quantity'] is None]
check('blind session hides system quantities from counters', len(masked) == len(r['lines']), (len(masked), len(r['lines'])))
c, r = call('GET', f'/api/inventory/{sid}', admin)
lines = r['lines']
check('admin sees system quantities', all(l['system_quantity'] is not None for l in lines))
# count every line at system value except our known batch (+2 variance)
target = next((l for l in lines if l['batch_id'] == new_batch), lines[0])
for l in lines:
...
c, r = call('GET', f'/api/master/batches?search=PO-R3-REALLOC', admin)
dst = next((b for b in r['batches'] if b['id'] == new_batch), {})
check('batch adjusted to counted quantity (4+2)', dst.get('remaining_quantity') == 6, dst.get('remaining_quantity'))
c, r = call('POST', '/api/receiving', supervisor, {'material_id': BOLT, 'received_quantity': 5,
...
for step in ('pack', 'load', 'dispatch'):
    c, r = call(f'POST', f'/api/shipping/{shp}/{step}', supervisor)
...
check('shipment shows POD + issued lines', r['shipment']['delivered_to'] == 'R. Receiver'
